...
export default function App() {
  ...
  useEffect(() => {
    ...
    const getMoistureValue = async (e) => {
      try {
        const response = await axios.get(`http://localhost:5000/get_moisture_value/lng=${lng}/lat=${lat}`);
        const responseMoisture = response.data.moisture;
        moisture.current = responseMoisture;
        console.log(`Fetched moisture value : ${responseMoisture} for coords (${lat}; `);
      } catch (error) {
        console.error(error);
      }
    };
